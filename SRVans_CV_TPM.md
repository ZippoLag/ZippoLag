# Sebastián Raúl Vansteenkiste  
Barcelona, Spain | EU Work Authorization  
- [LinkedIn: linkedin.com/in/srvans](https://www.linkedin.com/in/srvans/)
- [Email: sebavanten@gmail.com](mailto:sebavanten@gmail.com)
- [Book an interview with me](https://calendar.app.google/D6TQbH3ZZeuuNsP76) | [Read my Cover Letter](https://raw.githubusercontent.com/ZippoLag/ZippoLag/refs/heads/main/SRVans_CoverLetter.pdf)

---

## PROFILE

Senior Technical Program Manager with a strong software engineering background and extensive experience coordinating complex initiatives across distributed teams. Specialized in delivery execution, dependency management, technical risk mitigation, and cross-functional communication. Comfortable operating between engineering, product, and business stakeholders without relying on formal authority.

---

## CORE SKILLS

### Program & Delivery Management
- Technical program planning & execution  
- Dependency and risk management  
- Cross-team coordination  
- Roadmap and milestone tracking  
- Release & deployment orchestration  

### Stakeholder & Communication
- Business ↔ Engineering translation  
- Executive and stakeholder reporting  
- Requirements clarification  
- Conflict mediation  

### Technical Context (for decision-making)
- Backend systems (.NET, APIs, SQL)  
- Cloud platforms (Azure)  
- CI/CD pipelines  
- Distributed systems & integrations  
- Legacy system modernization contexts  

---

## RELEVANT EXPERIENCE

### Hitachi Energy — Barcelona, Spain  
**Technical Program Lead / Team Leader**  
*Sep 2024 – Dec 2025*

- Coordinated delivery of a globally deployed manufacturing traceability system involving multiple integrations and stakeholders.
- Planned and executed releases, hotfixes, and priority shifts based on user impact and operational risk.
- Led recurring ceremonies (planning, refinement, retrospectives) and tracked action items to completion.
- Acted as liaison between engineering teams, product, service, and business units.
- Identified process gaps and proposed incremental improvements to increase predictability and transparency.

### KopiusTech — Remote  
**Senior Engineer (Program Coordination Focus)**  
*Apr 2023 – Aug 2024*

- Coordinated technical work across multiple teams contributing to a micro-frontend enterprise platform.
- Helped identify requirement gaps early and mitigated delivery risks through documentation and alignment.
- Ensured smooth transitions between deployment versions across parallel initiatives.
- Supported onboarding and knowledge transfer to reduce delivery bottlenecks.

### ThinkerSys — Argentina / Remote  
**Senior Software Developer (Program-scale Projects)**  
*Jul 2018 – Jun 2021*

- Independently managed delivery of multiple concurrent projects for a large international NGO.
- Coordinated integrations between internal systems and third-party platforms.
- Gathered requirements directly from stakeholders and translated them into actionable plans.
- Managed deployments and operational concerns across cloud and self-hosted environments.

---

### Earlier Experience (Summary)

- **CloudX:** Multi-project coordination across rotating clients.  
- **Globant:** Team leadership in mobile initiatives.  
- **Accenture:** Scope item lead within large enterprise programs.

---

## EDUCATION

**Information Systems Engineering (coursework completed)**  
Universidad Tecnológica Nacional — Argentina  
**Information Technology Professional**
Complejo Ed. Dr. Francisco de Gurruchaga - Argentina

---

## LANGUAGES

- Spanish (Native)  
- English (Bilingual)  
- Japanese (Basic)  