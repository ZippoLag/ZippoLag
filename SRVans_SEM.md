# Sebastián Raúl Vansteenkiste  
Barcelona, Spain | EU Work Authorization  
- [LinkedIn: linkedin.com/in/srvans](https://www.linkedin.com/in/srvans/)
- [GitHub: github.com/zippolag](https://www.github.com/zippolag)
- [Email: sebavanten@gmail.com](mailto:sebavanten@gmail.com)
- [Book an interview with me](https://calendar.app.google/D6TQbH3ZZeuuNsP76) | [Read my Cover Letter](https://raw.githubusercontent.com/ZippoLag/ZippoLag/refs/heads/main/SRVans_CoverLetter.pdf)

---

## PROFILE

Software Engineering Manager with 10+ years in professional software development and recent hands-on experience leading distributed, multicultural teams in enterprise environments. Strong technical background in .NET and cloud-based systems, combined with a pragmatic, people-first leadership style. Proven track record stabilizing inherited systems, improving delivery processes, and aligning engineering execution with business priorities.

---

## CORE SKILLS

### Engineering Leadership
- Team leadership & mentoring  
- Line management (1:1s, performance reviews)  
- Hiring & technical interviewing  
- Conflict resolution  
- Remote & multicultural team management  

### Delivery & Process
- Agile / Scrum execution  
- Sprint planning & prioritization  
- Release & hotfix coordination  
- Risk management  
- Process documentation & improvement  

### Technical Background
- C# / .NET (Framework 2-4.5, Core, .NET 6–8)  
- Backend APIs (REST, GraphQL)  
- Azure (App Services, Functions, Queues, CosmosDB)  
- SQL (SQL Server, MySQL)  
- CI/CD (Azure DevOps)  

---

## PROFESSIONAL EXPERIENCE

### Hitachi Energy — Barcelona, Spain  
**Software Engineering Manager / Team Leader**  
*Sep 2024 – Dec 2025*

- Led a multicultural and distributed, mostly-remote full-stack team maintaining and extending a mission-critical manufacturing traceability system.
- Conducted regular 1:1s, performance follow-ups, and resolved interpersonal and delivery conflicts.
- Coordinated sprint planning, prioritization, regular releases, and hotfix deployments.
- Acted as primary engineering point of contact for product, service, and business stakeholders.
- Documented existing development processes and incrementally improved team execution and predictability.

---

### KopiusTech — Remote  
**Senior Full-Stack Developer (Technical Lead responsibilities)**  
*Apr 2023 – Aug 2024*

- Acted as de-facto technical lead for a Blazor micro-frontend initiative replacing a legacy enterprise system.
- Coordinated work across multiple teams and initiatives to avoid duplication and deployment conflicts.
- Worked closely with Product Owners to clarify requirements and anticipate technical risks.
- Produced extensive technical and onboarding documentation to reduce team dependency on tribal knowledge.
- Participated in technical interviews and internal training initiatives.

---

### Svitla — Remote  
**Senior .NET Backend Developer**  
*Jul 2022 – Mar 2023*

- Contributed to a regulated financial platform built on Azure serverless technologies.
- Improved API consistency and documented system behavior and business logic.
- Collaborated closely with cross-functional stakeholders to stabilize early-stage platform components.

---

### Earlier Experience (Summary)

- **CloudX (2021–2022):** Full-stack development across multiple client projects.  
- **ThinkerSys (2018–2021):** Independent senior developer for NGO and enterprise systems.  
- **Globant (2017–2018):** Xamarin Technical / Team Lead.  
- **Accenture (2014–2017):** Software Engineering Analyst / Associate.

---

## EDUCATION

**Information Systems Engineering (coursework completed, degree not pursued)**  
Universidad Tecnológica Nacional — Argentina  
**Information Technology Professional**
Complejo Ed. Dr. Francisco de Gurruchaga - Argentina

---

## LANGUAGES

- Spanish (Native)  
- English (Bilingual)  
- Japanese (Basic)  